...
export function SettingsDialog() {
  ...
  const handleSaveConfig = async () => {
    ...
    try {
      ...
      toast.error("Kesalahan saat memperbarui konfigurasi");
    } finally {
      setIsSaving(false);
    }
  };
  ...
  const handleDeleteAllThreads = async () => {
    if (!apiUrl) {
      toast.error("URL API tidak dikonfigurasi");
      return;
    }
    ...
    if (threadsToDelete.length === 0) {
      toast.info("Tidak ada percakapan untuk dihapus");
      return;
    }
    ...
    const confirmed = window.confirm(
      ...
      `Semua ${threadsToDelete.length} percakapan akan dihapus secara permanen.\n\n` +
    ...
    try {
      ...
      toast.success(`Berhasil menghapus ${threadsToDelete.length} percakapan`);

      // Reload the page to reset the chat interface
      window.location.reload();
    } catch (err) {
      console.error("Error deleting threads:", err);
      toast.error("Gagal menghapus semua percakapan");
    } finally {
      setIsDeleting(false);
    }
  };
    ...
    <Dialog>
      ...
      <DialogContent className="max-w-5xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Sesuaikan pengalaman obrolan Anda
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-6 py-4">
          ...
          <div className="space-y-3">
            ...
            <div className="space-y-4 rounded-lg border p-4">
              <div className="space-y-2">
                <Label htmlFor="font-family">Gaya Font</Label>
              ...
              <div className="space-y-2">
                <Label htmlFor="color-scheme">Skema Warna</Label>
                <div className="flex gap-2">
                  <Button
                    variant={
          ...
          {/* UI Behavior Section */}
          <div className="space-y-3">
            <h3 className="text-lg font-semibold">Perilaku UI</h3>
            <div className="space-y-4 rounded-lg border p-4">
              <div className="flex items-center justify-between">
                <div className="space-y-0.5">
                  <Label htmlFor="auto-collapse">Sembunyikan Panggilan Alat Otomatis</Label>
                  <p className="text-sm text-muted-foreground">
                    Sembunyikan detail panggilan alat secara otomatis setelah respons selesai
                  </p>
                </div>
                <Switch
                  id="auto-collapse"
              ...
              <div className="space-y-2">
                <Label htmlFor="chat-width">Lebar Obrolan</Label>
                <div className="flex gap-2">
                  <Button
                    variant={
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              <div>
                <h3 className="text-lg font-semibold">Konfigurasi Asisten</h3>
                {assistantId && (
            ...
            {isLoading ? (
                    ...
                    return (
                      <div key={key} className="space-y-2">
                        ...
                        {fieldType === 'boolean' ? (
                          <div className="flex items-center justify-between">
                            <p className="text-sm text-muted-foreground">
                              {schemaObj.description || `Alihkan ${schemaObj.title || key}`}
                            </p>
                            <Switch
                              id={`config-${key}`}
              ...
              <div className="rounded-lg border p-4">
                <p className="text-sm text-muted-foreground">
                  Tidak ada parameter yang dapat dikonfigurasi
                </p>
              </div>
            )}
          </div>
          ...
          <div className="space-y-3">
            ...
            <div className="rounded-lg border border-destructive/50 p-4">
              <div className="flex items-center justify-between">
                <div className="space-y-0.5">
                  <Label className="text-base">Hapus Semua Percakapan</Label>
                  <p className="text-sm text-muted-foreground">
                    Hapus semua riwayat percakapan secara permanen. Tindakan ini tidak dapat dibatalkan.
                  </p>
                </div>
                <Button
                  variant="destructive"
