...
export function Thread() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      toast.error("Terjadi kesalahan. Silakan coba lagi.", {
        description: (
          <p>
            <strong>Kesalahan:</strong> <code>{message}</code>
          </p>
        ),
        richColors: true,
        closeButton: true,
  ...
  const handleSubmit = (e: FormEvent) => {
    ...
    // Find the last image URL from content blocks if available
    const lastImageBlock = [...contentBlocks]
      .reverse()
      .find((block) => block.type === "image" && block.metadata?.url);
    const currentImageUrl = lastImageBlock?.metadata?.url as string | undefined;
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <TooltipProvider>
                            <Tooltip>
                              ...
                              <TooltipContent side="top">
                                <p>{hideToolCalls ? "Tampilkan panggilan alat" : "Sembunyikan panggilan alat"}</p>
                              </TooltipContent>
                            </Tooltip>
                          </TooltipProvider>
