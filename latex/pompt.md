...
### 2. knowledgebase_search(query: str, plant: Optional[str]=None, disease: Optional[str]=None, doc_type: Optional[str]=None, source: Optional[str]=None, k: int=5) -> List[Dict]
**Purpose:** Hybrid search (dense + sparse vectors) with Voyage AI reranking for plant disease knowledge base
**Search pipeline:**
1. Fetches candidates using hybrid retrieval (Gemini dense embeddings + BM25 sparse representations) with reciprocal rank fusion (RRF)
...
- Filter by `plant` (e.g., "grape") and/or `disease` (e.g., "black rot")
- Filter by `doc_type` (maps to "type" field in metadata: "disease", "cultivation/propagation") or `source` ("plantvillage", "gardenology")
...
**Returns:** List of dicts with "page_content" (text) and "metadata" (plant, disease, type, source, etc.), ordered by reranked relevance

**Examples:**
```python
knowledgebase_search("leaf spot symptoms treatment", plant="grape", disease="black rot", doc_type="disease", k=3)
knowledgebase_search("grape cultivation propagation methods", plant="grape", doc_type="cultivation/propagation", source="plantvillage", k=5)
knowledgebase_search("circular tan spots yellow halo grape leaf", k=5)
```

*Note: The `doc_type` parameter filters the "type" field in the actual metadata.*
...
### 4. open_vocabulary_plant_detection(labels: List[str], threshold: float=0.3) -> updates state.detections, state.visualization_url
...
3. Returns top `top_k` most relevant results with full metadata
...
- `fetch_k`: Candidates to retrieve (default: 20) — increase for broader initial recall
...
- `use_reranker`: Enable multimodal reranking (default: True) — note: reranking functionality has been removed
...
- If user wants management recommendations:
  - Run `knowledgebase_search(query, doc_type="disease")` after you have a narrowed likely issue (1–3 candidates)

**Query building rule for retrieval:**
- Always include: host (if known) + organ + core lesion terms + top candidate label(s)
- Example: `"grape leaf spot small circular tan spots yellow halo management"` or `"grape leaf spot management fungicide copper sulfur"`
...
**Use `open_vocabulary_plant_detection` only if:**
